...
/* 4.19'da struct msdc_host.power_switch `int (*)(...)`; 4.14 surumu
 * void donuyordu. Imza uyarlandi. */
...
int msdc_of_parse(struct platform_device *pdev, struct mmc_host *mmc)
{
	...
	/* selene: yalnizca eMMC (msdc0). msdc1 (SD kart yuvasi) probe u
	 * "DT probe msdc1!" satirindan ~49 us sonra NULL deref (0x4) ile
	 * kernel i dusuruyordu; eMMC probe u ise BASARILI (bootdevice).
	 * Boot icin SD karta ihtiyac yok, o yuzden atlaniyor.
	 */
	if (id != 0) {
		pr_notice("[msdc] selene: msdc%d ATLANDI (yalnizca eMMC)", id);
		return -ENODEV;
	}
	...
	/* selene/4.19: req_vcore VCORE OPP tabanini PM QoS ile sabitliyordu.
	 * 4.19 ComboA cekirdeginde ne `vcore_opp`/`req_vcore` uyeleri ne de
	 * PM_QOS_VCORE_OPP var. Bu bir DVFS voltaj-tabani optimizasyonu;
	 * eMMC onsuz calisir, o yuzden devre disi birakildi.
	 * (Stok DTB'de `req_vcore = <0x01>` duruyor, yok sayiliyor.)
	 */
	/* iomap register */
